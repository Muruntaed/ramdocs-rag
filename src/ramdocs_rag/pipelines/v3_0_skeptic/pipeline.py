"""V3.0 — Entity-First + Skeptic agent.

Flow: retrieve → analyze ×K → group_by_entity → reliability (2-pass)
      → per-entity intra-vote → DRAFT FinalAnswer
      → Skeptic verifies each variant against the full corpus
      → VERIFIED FinalAnswer.

The Skeptic is the only architectural difference vs v2.0. It runs once per
question (single-pass, not iterative), sees every document and every draft
variant, and decides keep/reject for each variant.
"""
...
class V3Skeptic(Pipeline):
    """v3.0: v2.0 + Skeptic verification step."""
    ...
    def __init__(
        ...
        # Skeptic defaults to the mediator's LLM (typically gpt-4o) because the
        # verification task benefits from a stronger model.
    ...
    def run(self, question: Question) -> RunResult:
        ...
        # 6. Group weights + relative cutoff
        group_weights = {
        ...
        # 7. Per-group resolution → draft variants
        variants = []
        ...
        # 8. Skeptic verification (NEW in v3.0)
